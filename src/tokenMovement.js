...
import { positionsAreEqual } from "./positions";
 ...
 * </Grid>
 */
export class TokenMovement extends React.Component {
  static propTypes = {
    x: PropTypes.number,
    y: PropTypes.number,
    z: PropTypes.number,
   ...
   */
  componentWillMount() {
    const coords = window.coords || { x: 0, y: 0, z: 0 };
  ...
  render() {
    return (
      <Hex
        ...
        onClick={() => {
          this.props.onSelectToken && this.props.onSelectToken();
        }}
      >
        {this.props.children}
      </Hex>
